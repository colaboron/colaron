...
userlist=['gianni.falzone@oracle.com']
counter = 5
for user in userlist:
    logify_nulls(user)
    url = "http://localhost:%s/sql" % str(22212 + int(cw))
    spayload = {'sql' : 'select persona_bm_id from sw_lead where email is not \'\'', 'user' :  user}
    resp  = requests.post(url, data=spayload)
    html = resp.text
    if html.find('<td>') == -1:
        print url, spayload
    ...
    print idlist, "LIST OF PBMID"
for item in idlist:
    for oitem in item.values()[0]:
        ebi = email_by_id(item.keys()[0], oitem, logger)
        if not ebi:
            print "found persona bm id without linkedin mail %s " % oitem
            ...
            user = item.keys()[0]
            email = property_by_pid(item.keys()[0], oitem, 'FULLEMAIL')
